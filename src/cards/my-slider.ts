...
export class MySliderV2 extends LitElement {
    ...
    private colorMode = 'brightness'
    private coverMode = 'position'
    private sliderId = ''
    private sliderEl: HTMLBodyElement | undefined
    private touchInput = false
    private disableScroll = true
    private allowTapping = true
    private thumbTapped = false
    private actionTaken = false
    private vertical = false
    private flipped = false
    private inverse = false
    private showMin = false
    private zero = 0
    private savedMin = 1
    private min = 0
    private max = 100
    private minThreshold = 0
    private maxThreshold = 100
    private step = 1
    private sliderVal = 0
    private sliderValPercent = 0.00
	...
	private _setColorTemp(entity, value): void {
        const oldVal = parseFloat(entity.attributes.color_temp)
	...
	private _setSwitch(entity, value): void {
		const threshold = Math.min(this.max, this.maxThreshold) //pick lesser of the two
	...
	private _setLock(entity, value): void {
		const threshold = Math.min(this.max, this.maxThreshold) //pick lesser of the two
		if (Number(threshold) <= value) {
			const newLockState = entity.state === "locked" ? 'unlock' : 'lock'
...
function styleMap(containerStl: any): unknown {
